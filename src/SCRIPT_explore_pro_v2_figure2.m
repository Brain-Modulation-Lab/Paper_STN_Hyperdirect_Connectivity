...
%design filter
%d1 = designfilt('bandstopiir','FilterOrder',20, 'HalfPowerFrequency1',55,'HalfPowerFrequency2',65, 'DesignMethod','butter','SampleRate',pro.fsample);
%d2 = designfilt('lowpassiir','FilterOrder',4,'HalfPowerFrequency',0.15,'DesignMethod','butter');
%d3 = designfilt('bandpassfir','FilterOrder',20, 'CutoffFrequency1',1,'CutoffFrequency2',40, 'SampleRate',pro.fsample); %best one, but I want freq>61 too
%d4 = designfilt('bandstopfir','FilterOrder',20, 'CutoffFrequency1',59,'CutoffFrequency2',61,   'SampleRate',pro.fsample);
d3 = designfilt('bandpassfir','FilterOrder',20, 'CutoffFrequency1',300,'CutoffFrequency2',400, 'SampleRate',30000);

quickplot = 0;

for sett = 9% [1 2 4 5 6 7 8 9]
    ...
    % pick your option
    pplotalltrials = 1 ; %pplotalltrials=0 --> plot all channels, pplotalltrials=1 --> plot all trials for one channel,
    ...
    for choi = choix
        ...
        for i = 1:plotuptotrials
            ...
            %settings 2
            indx2_f = 0.014;
            indx2 = find(pro.time{indx(1)}>0.0005  & pro.time{indx(1)}<=indx2_f);     %this time in seconds
            ...
            if pplotalltrials == 1 && quickplot == 0
        ...
        if pplotalltrials == 0 && quickplot == 0
